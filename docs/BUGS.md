...
| B-004 | `esp_cam_sensor` 1.5.2 的私有 `spi_slave.c` 无法在 ESP-IDF 6.0.3 编译（`spicommon_dma_chan_alloc` 参数与 `hal->bitlen` 已变） | `idf.py build` @ `C:\esp\v6.0.3\esp-idf` | F-080 | fixed |
